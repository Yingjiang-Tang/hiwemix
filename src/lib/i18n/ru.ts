// ru 语言文案
import { dict } from './_helpers';

const dict_ru = dict({
    brandName: "HIWE Поиск формул",
    brandNameShort: "HIWE",
    heroTitlePrefix: "Найдите своё",
    heroTitleHighlight: "идеальное совпадение",
    heroBadge: "Профессиональная система формул автоэмалей",
    heroSubtitle: "Точные формулы по марке, году или коду цвета.",
    heroTrust1: "500 000+ формул",
    heroTrust2: "50+ автобрендов",
    heroTrust3: "Глобальная база красок",
    heroCta: "Исследовать",
    login: "Войти",
    close: "Закрыть",
    brandSlogan: "СИСТЕМА ПОИСКА АВТОЭМАЛЕЙ",
    officialWebsite: "Официальный сайт",
    loginMobileTitle: "Добро пожаловать в HAIWEN",
    tdsSearchPlaceholder: "Поиск руководств...",
    tdsCategories: "Категории",
    tdsAllCategories: "Все категории",
    tdsListLabel: "Список руководств",
    tdsSelectHint: "Выберите руководство слева",
    tdsBackToList: "← Back to all documents",
    tdsTableOfContents: "Navigation",
    tdsDocTypeAll: "Все типы",
    tdsDocTypeTds: "Технический паспорт",
    tdsDocTypeMsds: "Безопасность материала",
    tdsDocTypeSds: "Паспорт безопасности",
    tdsDocTypeManual: "Руководство",
    navSearch: "Поиск", navProducts: "Продукты", navAbout: "О нас",
    navFormulaSearch: "Поиск формул",
    navColorLibrary: "Пигменты",
    navFavorites: "Избранное",
    removeFavorite: "Удалить",
    favoritesTitle: "Мои избранные",
    favoritesEmpty: "Пока нет избранных",
    favoritesEmptyHint: "Нажмите на сердечко у любой формулы, чтобы сохранить её здесь",
    favoritesSearchPlaceholder: "Поиск кода цвета, названия или марки...",
    navTds: "Документы",
    navAdmin: "Управление данными",
    userManagement: "Управление пользователями",
    logout: "Выход",
    forgotPassword: "Забыли пароль?",
    or: "или",
    continueWithGoogle: "Продолжить с Google",
    continueWithFacebook: "Продолжить с Facebook",
    noAccount: "Нет аккаунта?",
    signUp: "Регистрация",
    loginErrorInvalid: "Неверный email или пароль. Если вы регистрировались через Google или Facebook, используйте кнопки ниже.",
    loginResetSuccess: "Пароль обновлён. Войдите с новым паролем.",
    oauthGoogleFailed: "Не удалось войти через Google",
    oauthFacebookFailed: "Не удалось войти через Facebook",
    oauthUnavailable: "Вход через соцсети временно недоступен",
    loginRegisterLink: "Нет аккаунта? Регистрация",
    registerWelcome: "Добро пожаловать",
    registerSubtitle: "Создайте аккаунт, чтобы начать",
    registerTitle: "Создать аккаунт",
    registerButton: "Зарегистрироваться",
    registerConfirmLabel: "Подтвердите пароль",
    registerConfirmPlaceholder: "Повторите пароль",
    registerPasswordPlaceholder: "Не менее 8 символов",
    registerConfirmEmail: "Аккаунт создан. Подтвердите email и войдите.",
    registerSuccess: "Регистрация успешна, вход...",
    backToLogin: "Назад ко входу",
    haveAccount: "Уже есть аккаунт?",
    loginLink: "Войти",
    registerErrorExists: "Такое имя пользователя уже существует",
    registerErrorFormat: "Имя 3-20 символов: начинается с буквы, только буквы/цифры/_",
    registerErrorPassword: "Пароль должен быть не менее 8 символов",
    registerErrorMismatch: "Пароли не совпадают",
    registerLoginLink: "Уже есть аккаунт? Войти",
    registerErrorFailed: "Ошибка регистрации",

    loginWelcome: "С возвращением",
    loginSubtitle: "Введите свои учётные данные для входа",
    loginEmail: "Эл. почта", loginPassword: "Пароль",
    loginPlaceholderEmail: "вы@пример.com",
    loginPlaceholderPassword: "Введите пароль",
    loginButton: "Начать", loginSigningIn: "Вход...",
    loginErrorEmpty: "Введите имя пользователя и пароль",
    loginErrorNetwork: "Ошибка сети, повторите попытку",
    loginErrorFailed: "Не удалось войти",
    panelTitle: "Поиск формул",
    make: "Марка", colorCode: "Код цвета", colorName: "Название цвета", colorType: "Тип цвета",
    allMakes: "Все марки",
    colorTypeAll: "Все", colorTypeSolid: "Однотонный", colorTypeMetallic: "Металлик",
    colorTypePearl: "Перламутр", colorTypeMatte: "Матовый", colorTypeCandy: "Канди",
    search: "Поиск", searching: "Поиск...", reset: "Сброс",
    codeTooLong: "Код цвета обычно <= 10 символов",
    colorCodePlaceholder: "напр. 040, NH731P",
    colorNamePlaceholder: "напр. Super White",
    year: "Год", yearPlaceholder: "напр. 2020 или 2018-2022",
    yearSingle: "Один", yearRange: "Диапазон",
    detail: "Подробнее", expand: "Развернуть", collapse: "Свернуть", viewMore: "Показать больше",
    version: "Версия", paintSystemNotes: "Примечания",
    volume: "Объём", tonerCode: "Код тонера", tonerName: "Название тонера",
    percentage: "Процент(%)", actualAmount: "Факт. кол-во(г)",
    colorInfo: "Информация о цвете", formulaVariants: "Варианты формулы", components: "Компоненты",
    makeLabel: "Марка", typeLabel: "Тип", yearsLabel: "Годы", codeLabel: "Код",
    print: "Печать", copy: "Копировать", notesLabel: "Примечания", updatedLabel: "Обновлено",
    tabColorInfo: "Информация о цвете", manufacturerLabel: "Производитель", originLabel: "Происхождение",
    carModelLabel: "Модель", processLabel: "Процесс", versionLabel: "Версия",
    weight: "Вес", accum: "Накоплено", massTone: "Тон массы",
    totalWeightLabel: "Итого", pearlPaintLabel: "Перламутровая краска", groundPaintLabel: "Грунтовка",
    colorTypeSolidLabel: "Однотонный", colorTypeMetallicLabel: "Металлик", colorTypePearlLabel: "Перламутр",
    colorTypeMatteLabel: "Матовый", colorTypeCandyLabel: "Канди", colorTypeSpecialLabel: "Специальный",
    copySuccess: "Скопировано в буфер обмена", copyFail: "Не удалось скопировать",
    searchHint: "Введите критерии поиска слева",
    noResults: "Соответствующих цветов не найдено",
    noResultsHint: "Попробуйте другую марку или код цвета",
    adminTitle: "Список пользователей",
    adminNewUser: "Новый пользователь",
    adminNoPermission: "Нет доступа",
    adminPasswordRequired: "Для нового пользователя пароль обязателен",
    adminCannotDeleteAdmin: "Нельзя удалить суперадминистратора",
    adminLoading: "Загрузка...",
    adminColId: "ID",
    adminColUsername: "Имя пользователя",
    adminColRole: "Роль",
    adminColCreatedAt: "Создан",
    adminColActions: "Действия",
    adminRoleAdmin: "Админ",
    adminRoleUser: "Пользователь",
    adminEdit: "Изменить",
    adminDelete: "Удалить",
    adminEditTitle: "Изменить пользователя",
    adminLabelPassword: "Пароль",
    adminPasswordHint: "Оставьте пустым, чтобы не менять",
    adminPasswordPlaceholder: "Оставьте пустым, чтобы сохранить",
    adminLabelRole: "Роль",
    adminCancel: "Отмена",
    adminSave: "Сохранить",
    adminCreate: "Создать",
    adminConfirmDelete: (username: string) => `Вы уверены, что хотите удалить пользователя "${username}"?`,
    formulasCount: (n) => {
      const mod10 = n % 10, mod100 = n % 100;
      const word = mod10 === 1 && mod100 !== 11 ? "формула"
                 : mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14) ? "формулы"
                 : "формул";
      return `${n} ${word}`;
    },
    foundCount: (n) => {
      const mod10 = n % 10, mod100 = n % 100;
      const word = mod10 === 1 && mod100 !== 11 ? "цвет"
                 : mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14) ? "цвета"
                 : "цветов";
      return `Найдено ${n} ${word}`;
    },
    truncatedHint: (max) => `Показаны первые ${max} результатов. Уточните поиск.`,
    totalFormula: (c, f) => `${c} цветов, ${f} формул`,
    colorsBadge: (n) => `${n} Цв.`,
    formulasBadge: (n) => `${n} Формул`,
    navMyFormulas: "Мои формулы",
    navCompare: "Сравнить",
    cancel: "Отмена",
    saveFormula: "Сохранить формулу",
    savedFormula: "Сохранено",
    saveFormulaDialogTitle: "Сохранить формулу",
    saveFormulaNameLabel: "Название",
    saveFormulaNamePlaceholder: "напр. Toyota | 040 Супер Белый",
    saveFormulaSuccess: "Формула сохранена",
    saveFormulaFail: "Не удалось сохранить, попробуйте снова",
    saveRequireLogin: "Войдите, чтобы сохранять формулы",
    myFormulasTitle: "Мои формулы",
    myFormulasEmpty: "Нет сохранённых формул",
    myFormulasEmptyHint: "Откройте формулу и нажмите Сохранить, чтобы оставить её здесь",
    myFormulasDelete: "Удалить",
    compareTitle: "Сравнить формулы",
    addToCompare: "Добавить к сравнению",
    inCompare: "В сравнении",
    addedToCompare: "Добавлено к сравнению",
    removedFromCompare: "Удалено из сравнения",
    compareEmpty: "Список сравнения пуст",
    compareEmptyHint: "Откройте формулы и нажмите Сравнить, чтобы добавить их сюда",
    compareGrams: "г/100г",
    compareOnlyIn: "Только в",
    compareClearAll: "Очистить всё",
});

export default dict_ru;
