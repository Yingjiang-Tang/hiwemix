...
export default function FormulaDrawer({ result, onClose, initialFormulaIdx, formulaId, initialYear }: FormulaDrawerProps) {
  ...
  // 加入 / 移出对比篮（toggle；toast 文案按加入前状态取）
      ...
      <Sheet open onOpenChange={(v) => { if (!v) handleClose(); }}>
        <SheetContent side="right" className="formula-print-area formula-drawer p-0 gap-0 bg-card z-[2000] md:!fixed md:!inset-0 md:!w-screen md:!max-w-none md:!h-screen md:!translate-x-0 md:!rounded-none md:!overflow-y-auto md:!z-[2000]">
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-card px-6 py-4 md:px-[60px] md:py-5">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="hidden md:flex relative left-[-10px] items-center gap-2.5 flex-shrink-0">
                ...
                <Button
                  ...
                  size="icon"
                  className={`size-[38px] rounded-full bg-transparent border-muted-foreground/30 ${
                    isInCompare(activeFormula?.id ?? "") ? "border-muted-foreground/30 bg-muted text-foreground" : "text-muted-foreground hover:text-foreground"
                  }`}
                  aria-pressed={isInCompare(activeFormula?.id ?? "")}
                  aria-label={isInCompare(activeFormula?.id ?? "") ? t.inCompare : t.addToCompare}
                >
            ...
            {/* 移动端操作按钮：第二行（Favorite/保存/复制/对比；无打印，见用户要求） */}
